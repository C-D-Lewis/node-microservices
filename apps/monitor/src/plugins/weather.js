const {
  requestAsync, config, fcm, conduit, log
} = require('@chris-lewis/node-common')(['requestAsync', 'config', 'fcm', 'conduit', 'log']);
...
const checkWeather = async (args) => {
  ...
  if(isRaining) state = config.LED_STATES.RAIN;
  else if(temperature < args.TEMP_COLD) state = config.LED_STATES.COLD;
  else if(temperature > args.TEMP_HOT) state = config.LED_STATES.HOT;
...
module.exports = async (args) => {
  ...
  try {
    ...
    conduit.send({
      to: 'LedServer', topic: 'setPixel',
      message: { [args.LED]: config.LED_STATES.DOWN }
    });
  }
};
